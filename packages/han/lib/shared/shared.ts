 ...
 */
const PLUGIN_DETECTION_SCHEMA = {
  type: 'object',
  properties: {
    plugins: {
      type: 'array',
      items: { type: 'string' },
      description: 'Array of recommended plugin names from the Han marketplace',
    },
    reasoning: {
      type: 'string',
      description: 'Brief explanation of why these plugins were recommended',
    },
  },
  required: ['plugins'],
} as const;
...
export async function detectPluginsWithAgent(
  ...
  try {
    ...
    // Parse structured JSON response from outputFormat
    interface PluginDetectionResponse {
      plugins: string[];
      reasoning?: string;
    }
    ...
    let plugins: string[] = [];
    try {
      const parsed = JSON.parse(responseContent) as PluginDetectionResponse;
      plugins = parsed.plugins || [];
    } catch {
      // Fallback to regex parsing if JSON parsing fails
      console.warn(
        '[Plugin Detection] Response was not valid JSON, falling back to regex parsing'
      );
      plugins = parsePluginRecommendations(responseContent);
    }
